    ...
    api.on("before_prompt_build", async (event: any, ctx?: PluginHookAgentContext) => {
      try {
        ...
        const results = pluginConfig.recallTopK
          ? dedupedResults.slice(0, pluginConfig.recallTopK)
          : dedupedResults;

        debug(
          `[Hindsight] After dedup+topK (${pluginConfig.recallTopK ?? "unlimited"}): ${response.results.length} raw → ${dedupedResults.length} deduped → ${results.length} injected`
        );
